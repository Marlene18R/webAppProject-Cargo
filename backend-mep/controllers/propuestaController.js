...
const getPropuestas = async (req, res) => {
  try {
    ...
    const [rows] = await pool.query(
      'SELECT * FROM vista_propuestas_escuela WHERE id_escuela = ?',
      [id]
    );
    res.json(rows);
  } catch (error) {
...
const addPropuesta = async (req, res) => {
  try {
    const { id } = req.params;
    const { subcategoria, categoria, propuesta, cantidad_requerida, unidad, estado, detalles } = req.body;
    
    const [result] = await pool.query(
      'CALL sp_upsert_propuesta(?, ?, ?, ?, ?, ?, ?, ?)',
      [id, subcategoria, categoria, propuesta, cantidad_requerida, unidad, estado, detalles]
    );
...
const updatePropuesta = async (req, res) => {
  try {
    ...
    await pool.query(
      'CALL sp_update_propuesta(?, ?, ?, ?, ?, ?, ?, ?)',
      [idNecesidad, subcategoria, categoria, propuesta, cantidad_requerida, unidad, estado, detalles]
    );
