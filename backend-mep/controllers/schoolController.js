...
const createSchool = async (req, res) => {
  try {
    ...
    res
      .status(201)
      .json({ message: 'Escuela creada exitosamente', id: id_escuela });
  } catch (error) {
    console.error(error);
...
const updateSchool = async (req, res) => {
  ...
  try {
    const [existing] = await pool.query(
      'SELECT id_escuela FROM escuela WHERE id_escuela = ?',
      [id]
    );
    ...
    if (fields.length > 0) {
      values.push(id);
      await pool.query(
        `UPDATE escuela SET ${fields.join(', ')} WHERE id_escuela = ?`,
        values
      );
    }
    ...
    if (updates.donationTypes && Array.isArray(updates.donationTypes)) {
      await pool.query(
        'DELETE FROM escuela_tipo_donacion WHERE id_escuela = ?',
        [id]
      );
...
const uploadSchoolImage = async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ message: 'Error al actualizar imagen de la escuela' });
  }
};
